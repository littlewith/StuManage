// StuFinal.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
int main()
{
 UI ui; File file;  All_students allstudents;
	ui.logo_print();
	ui.loading();
	file.fetch_db_init();
	file.fetch_db_input(allstudents);
	int identify=2;
	int failed_count = 0;
	...
	string oldpasswd="";
	do {
		string* logindata = ui.login();//登录
		identify = file.fetch_db_login(logindata[0], logindata[1]);
		oldusername = logindata[0];
		oldpasswd = logindata[1];
		failed_count++;
		if (identify >= 2) {
			ui.Login_failed(failed_count);
		}
	} while (identify >= 2);
	//cout << identify;
	/*
	cout << "============================"<<endl;
	cout << oldusername << endl;;
	cout << oldpasswd << endl;;
	cout << "==========================================" << endl;
	system("pause");
	*/
	while (true) {
		switch (identify) {//一级菜单
		case 0:
			while (true) {
				int choose = ui.Welcome_admin(&oldusername);
				if (choose == 1) {
					while (true) {
						int choose = ui.Welcome_control_student();
						...
						if (choose == 1) {
							...
							allstudents.chg();
								...
								system("cls");
								allstudents.find_student();
							
						}
						else if (choose == 5) {
							break;
						}
						else if (choose == EXIT_SYSTEM_INFO) {
							return 0;
						}
						system("cls");
					}
					...
					while (true) {
						int choose = ui.Welcoem_control_class();
						if (choose == 1) {
							system("cls");
							allstudents.print();
					...
					while (true) {
						int choose = ui.Welcome_change_info();//管理员更换账户信息
						if (choose == 1) {
							system("cls");
							cout << "\t\t请输入新的密码:";
							string new_passwd;
							cin >> new_passwd;
							file.fetch_db_chg_save_account(oldusername, oldpasswd, oldusername, new_passwd);	
							cout << "\t\t用户信息更新完成！" << endl;
							Sleep(1000);
							break;
						}
							...
							string new_username;
							cin >> new_username;
							file.fetch_db_chg_save_account(oldusername, oldpasswd, new_username, oldpasswd);
							cout << "\t\t用户信息更新完成！" << endl;
							Sleep(1000);
							break;
						}
		...
		case 1:
			while (true) {
				int choose = ui.Welcome_student(&oldusername);
				if (choose == 1) {
						allstudents.find_students();	
				}
				else if (choose == 2) {
					while (true) {
						int choose = ui.Welcome_change_info();//管理员更换账户信息
